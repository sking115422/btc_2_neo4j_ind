...
#This file includes modification by elika.bozorgi@uga.edu-reading the data from an excel sheet
...
input_dir = "./result/"
index_path = "./index/block_index.csv"
...
try:
    ...
    for t in range (iter_start, 2):
        ...
        # testing loop below
        # for a in range(0, 1):
        # all the hash blocks with their times are in Final_hashlist, and now we are reading from it
        with open("result/Final_hashlist.csv") as final:
            final_hash = csv.reader(final)
            for row, item in enumerate(final_hash):
                if row == 0:
                    continue
                ...
                # implemented above: Reset bn_start each time DAT file finished    
                    ...
                    # df_start = a
                df_start = item[4]    
                dat_number = str(item[4]).rjust(5, '0')
                ...
                with open(input_dir + dat_name) as bl:  #reading the json file from the beginning
                    ...
                    for i in range(0, len(bl_json)):
                        ...
                        blk = bl_json[i]
                        #finding the bn_start by comparing the hash numbers of the current column with the current block
                        if item[1] == blk["hash"]:
                            bn_start = i
                        else:
                            continue
