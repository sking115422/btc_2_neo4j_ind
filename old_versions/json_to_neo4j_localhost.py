...
#This file includes modification by elika.bozorgi@uga.edu-Reading the data from the localhost
...
#creating a list that contains the block hashes, to amke sure we maintain the blocks in order
unranked_time = []
...
try:
    ...
    for t in range (iter_start, 2):
        ...
        # testing loop below
        # for a in range(0, 1):
        # all the hash blocks with their times are in the database in <table-name>, and now we are reading from it
        # you need to config accordingly
        conn = psycopg2.connect(database="<database-name>",
                        host= "localhost",
                        user="<user-name>",
                        password="<password>")
        cursor = conn.cursor()
        cursor.execute ("SELECT to_timestamp(unixtime) as date FROM <table-name> group by to_timestamp(unixtime) ORDER BY date")
        cursor.execute("SELECT * from <table-name>")
        data = cursor.fetchall()
        for row in data:
            dat_number = str(row[4]).rjust(5, '0')
            dat_name = "blocklist_" + str(dat_number) + ".json"
            if row == 0:
                continue
                    
                # implemented above: Reset bn_start each time DAT file finished    
                    # if a != df_start:
                    #     bn_start = 0
            ...
            with open(input_dir + dat_name) as bl:  #reading the json file from the beginning
                ...
                for i in range(0, len(bl_json)):
                        blk = bl_json[i]
                        #finding the bn_start by comaring the hash numbers of the current column with the current block
                        if row[1] == blk["hash"]:
                            bn_start = i
                        else:
                            continue
                        ...
                        if t == 0:
                            ...
                            unranked_time.append(time)
                            print("unranked time: " + unranked_time)
